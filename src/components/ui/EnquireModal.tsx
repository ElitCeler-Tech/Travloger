  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const getDestinationFromUrl = () => {
        ...
        if (path === '/' || path === '/kashmir') return 'Kashmir';
        if (path === '/ladakh') return 'Ladakh';
        if (path === '/kerala') return 'Kerala';
        if (path === '/gokarna') return 'Gokarna';
        if (path === '/meghalaya') return 'Meghalaya';
        if (path === '/mysore') return 'Mysore';
        if (path === '/singapore') return 'Singapore';
        return 'Unknown';
      };
