...
interface HyderabadTripHighlightsProps {
  content?: TripHighlightsContent;
}

const hyderabadHighlights: HighlightImage[] = [
  { 
    id: '1', 
    src: highlight1, 
    alt: 'Charminar - Iconic monument and symbol of Hyderabad'
  },
  { 
    id: '2', 
    src: highlight2, 
    alt: 'Golconda Fort - Ancient fortress with amazing acoustics'
  },
  { 
    id: '3', 
    src: highlight3, 
    alt: 'Hussain Sagar Lake - Heart of the city with Buddha statue'
  },
  { 
    id: '4', 
    src: highlight1, 
    alt: 'Chowmahalla Palace - Royal residence of Nizams'
  },
  { 
    id: '5', 
    src: highlight2, 
    alt: 'Salar Jung Museum - World-class art collection'
  },
  { 
    id: '6', 
    src: highlight3, 
    alt: 'Qutub Shahi Tombs - Royal necropolis and architectural marvel'
  },
];

const HyderabadTripHighlights = ({ content }: HyderabadTripHighlightsProps) => {
  ...
  const highlightsData = React.useMemo(() => {
    if (!content?.highlights || content.highlights.length === 0) {
      return hyderabadHighlights;
    }
    
    // Smart merging: combine default items with CMS items
    return hyderabadHighlights.map(defaultItem => {
        ...
        <LazyLoad animationType="fade" delay={0.2}>
          <div className="text-center mb-8 md:mb-12">
            <h2 className={cn(
              ...
              {content?.heading || (
                <>
                  Discover Hyderabad&apos;s{' '}
                  <span className="text-[#134956]">Hidden Gems</span>
                </>
              )}
            </h2>
            <p className="text-gray-600 text-base md:text-lg max-w-2xl mx-auto">
              {content?.subheading || "Experience the most breathtaking destinations that make Hyderabad truly magical"}
            </p>
          </div>
        </LazyLoad>
...
export default HyderabadTripHighlights;
