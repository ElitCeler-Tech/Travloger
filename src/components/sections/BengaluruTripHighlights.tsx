...
interface BengaluruTripHighlightsProps {
  content?: TripHighlightsContent;
}

const bengaluruHighlights: HighlightImage[] = [
  { 
    id: '1', 
    src: highlight1, 
    alt: 'Lalbagh Botanical Garden - 240-acre botanical paradise'
  },
  { 
    id: '2', 
    src: highlight2, 
    alt: 'Cubbon Park - Green lung of the city'
  },
  { 
    id: '3', 
    src: highlight3, 
    alt: 'Vidhana Soudha - Magnificent state legislature building'
  },
  { 
    id: '4', 
    src: highlight1, 
    alt: 'Bannerghatta National Park - Wildlife sanctuary'
  },
  { 
    id: '5', 
    src: highlight2, 
    alt: 'Nandi Hills - Scenic hill station with panoramic views'
  },
  { 
    id: '6', 
    src: highlight3, 
    alt: 'Bangalore Palace - Royal residence with Tudor architecture'
  },
];

const BengaluruTripHighlights = ({ content }: BengaluruTripHighlightsProps) => {
  ...
  const highlightsData = React.useMemo(() => {
    if (!content?.highlights || content.highlights.length === 0) {
      return bengaluruHighlights;
    }
    
    // Smart merging: combine default items with CMS items
    return bengaluruHighlights.map(defaultItem => {
        ...
        <LazyLoad animationType="fade" delay={0.2}>
          <div className="text-center mb-8 md:mb-12">
            ...
            <p className="text-gray-600 text-base md:text-lg max-w-2xl mx-auto">
              {content?.subheading || "Experience the most breathtaking destinations that make Bengaluru truly magical"}
            </p>
          </div>
        </LazyLoad>
...
export default BengaluruTripHighlights;
