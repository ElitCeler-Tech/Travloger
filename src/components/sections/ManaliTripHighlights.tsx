...
interface ManaliTripHighlightsProps {
  content?: TripHighlightsContent;
}

const manaliHighlights: HighlightImage[] = [
  { 
    id: '1', 
    src: highlight1, 
    alt: 'Solang Valley - Adventure sports hub with paragliding and skiing'
  },
  { 
    id: '2', 
    src: highlight2, 
    alt: 'Rohtang Pass - High altitude mountain pass with snow activities'
  },
  { 
    id: '3', 
    src: highlight3, 
    alt: 'Hidimba Temple - Ancient cave temple surrounded by cedar forests'
  },
  { 
    id: '4', 
    src: highlight1, 
    alt: 'Manikaran Sahib - Sacred hot springs and spiritual center'
  },
  { 
    id: '5', 
    src: highlight2, 
    alt: 'Kasol - Hippie paradise with scenic mountain views'
  },
  { 
    id: '6', 
    src: highlight3, 
    alt: 'Vashisht Temple - Natural hot water springs and ancient temple'
  },
];

const ManaliTripHighlights = ({ content }: ManaliTripHighlightsProps) => {
  ...
  const highlightsData = React.useMemo(() => {
    if (!content?.highlights || content.highlights.length === 0) {
      return manaliHighlights;
    }
    
    // Smart merging: combine default items with CMS items
    return manaliHighlights.map(defaultItem => {
        ...
        <LazyLoad animationType="fade" delay={0.2}>
          <div className="text-center mb-8 md:mb-12">
            ...
            <p className="text-gray-600 text-base md:text-lg max-w-2xl mx-auto">
              {content?.subheading || "Experience the most breathtaking destinations that make Manali truly magical"}
            </p>
          </div>
        </LazyLoad>
...
export default ManaliTripHighlights;
