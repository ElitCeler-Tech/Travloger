...
const HighlightedHeading = ({ text, highlightText }: { text: string; highlightText?: string }) => {
  ...
  if (highlightText && text.includes(highlightText)) {
    ...
    return (
      <>
        {parts[0]}<span className="relative text-[#134956]">{highlightText}<span className="absolute left-0 right-0 -bottom-1 h-[8px] bg-[#134956]/30 rounded-full skew-x-[-2deg]" /></span>{parts[1] || ''}
      </>
    );
  }
  ...
  if (text.includes('Explore ')) {
    ...
    return (
      <>
        {parts[0]}<span className="relative text-[#134956]">Explore {parts[1]}<span className="absolute left-0 right-0 -bottom-1 h-[8px] bg-[#134956]/30 rounded-full skew-x-[-2deg]" /></span>
      </>
    );
  }
  ...
  if (words.length > 3) {
    ...
    return (
      <>
        {words.join(' ')} <span className="relative text-[#134956]">{lastWord}<span className="absolute left-0 right-0 -bottom-1 h-[8px] bg-[#134956]/30 rounded-full skew-x-[-2deg]" /></span>
      </>
    );
  }
