...
const BengaluruTripOptions = React.memo(({ content }: { content?: TripOptionsContent }) => {
  ...
  const { packages: customTrips } = usePackages({ tripType: 'custom', destination: 'Bengaluru' });
  const { packages: groupTrips } = usePackages({ tripType: 'group', destination: 'Bengaluru' });
          ...
          <div className="text-center mb-8 md:mb-12">
            <h2 className={cn(
              ...
              {content?.heading ? (
                ...
                <>
                  How Do You Want To{' '}
                  <span className="relative text-[#134956]">Explore Bengaluru<span className="absolute left-0 right-0 -bottom-2 h-[18px] w-full" style={{backgroundImage: "url(/brush-underline.svg)", backgroundSize: "100% 100%", backgroundRepeat: "no-repeat"}} /></span>?
                </>
              )}
            </h2>
            {content?.subheading && (
